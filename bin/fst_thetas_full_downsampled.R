...
genome_length = 280e6
theta_file_list = list.files(pattern = '*form_nomaf*') #get all angsd dothetas output
#theta_file_list = theta_file_list[!grepl('form_nomaf', theta_file_list)] #remove ones we don't want
thetawindow = lapply(seq_along(theta_file_list), function(i){fread(theta_file_list[[i]]) %>% mutate(fn=theta_file_list[[i]])}) #fread all, adding filename as col
thetawindow = rbindlist(thetawindow) #bind into big df
thetawindow$form = gsub("_nomaf.*","",thetawindow$fn)
chrlens = aggregate(thetawindow$WinCenter, by = list(thetawindow$Chr), max) #get chromosome lengths (mx bp)
...
thetawindow = thetawindow[chrlens, on = .(Chr)] #join to thetas data
thetawindow$BPcum = thetawindow$WinCenter + thetawindow$cumsum #get cumulative position in genome
thetawindow = thetawindow %>% mutate(Pi = tP / nSites) # get pi normalised by snp num (see angsd docs)
?separate
#clean piece
thetawindow$piece = thetawindow$piece = gsub("_nomaf.*","",thetawindow$piece)
thetawindow$piece = thetawindow$piece = gsub('decid_forest', "decid-forest",  thetawindow$piece)
thetawindow$piece = thetawindow$piece = gsub('decidforest', "decid-forest",  thetawindow$piece)
#plot tajimas d
m_tajima = thetawindow %>% 
  ggplot(., aes(x=BPcum, y=Pi, colour = Chr))+
  geom_line(size=0.3)+
  theme_classic()+
  scale_color_brewer(palette='Set2')+
  annotate("text", x=221306572, y=4, label = 'Cyp9k1', angle = 0)+
  annotate("text", x=25363652, y=4, label = 'Rdl', angle = 0)+
  annotate("text", x=77840876, y=4, label = 'Cyp6aa1–6p2', angle = 0)+
  annotate("text", x=52843399, y=4, label = 'Ace1', angle = 0)+
  annotate("text", x=37291800, y=4, label = 'coeae5g', angle = 0)+
  annotate("text", x=3554800, y=4, label = '?gpcr', angle = 0)+
  labs(x='Cumulative Position', colour = 'Chromosome')

s_tajima = thetawindow %>% filter(form == 'sform') %>% 
  ggplot(., aes(x=BPcum, y=Tajima, colour = Chr))+
  geom_point(size=0.3)+
  theme_classic()+
  facet_wrap(~ piece, nrow = 5)+
  scale_color_brewer(palette='Set2')+
  geom_point(size=0.3)+
  annotate("text", x=221306572, y=4, label = 'Cyp9k1', angle = 0)+
  annotate("text", x=25363652, y=4, label = 'Rdl', angle = 0)+
...
m_pi = thetawindow %>% filter(form == 'mform') %>% 
  ggplot(., aes(x=BPcum, y=Pi, colour = Chr))+
  geom_point(size=0.3)+
  theme_classic()+
  facet_wrap(~ piece, nrow = 5)+
  scale_color_brewer(palette='Set2')+
  geom_point(size=0.3)+
  annotate("text", x=221306572, y=1, label = 'Cyp9k1', angle = 0)+
  annotate("text", x=25363652, y=1, label = 'Rdl', angle = 0)+
  annotate("text", x=77840876, y=1, label = 'Cyp6aa1–6p2', angle = 0)+
  annotate("text", x=52843399, y=1, label = 'Ace1', angle = 0)+
  annotate("text", x=37291800, y=1, label = 'coeae5g', angle = 0)+
  annotate("text", x=3554800, y=1, label = '?gpcr', angle = 0)+
  labs(x='Cumulative Position', colour = 'Chromosome')


s_pi = thetawindow %>% filter(form == 'sform') %>% 
  ggplot(., aes(x=BPcum, y=Pi, colour = Chr))+
  geom_point(size=0.3)+
  theme_classic()+
  facet_wrap(~ piece, nrow = 5)+
...
agamp4_x_pi = thetawindow %>% filter(Chr == 'AgamP4_X') %>% 
  ggplot(., aes(x=WinCenter, y=Tajima, colour = Chr))+
  geom_point(size=0.3)+
  theme_classic()+
  facet_wrap(~ piece + form)+
  scale_color_brewer(palette='Set2')
ggsave('X_tajima.tiff', agamp4_x_pi, width = 12, height = 8, units = 'in', device = grDevices::tiff)


agamp4_x_taj = thetawindow %>% filter(form == 'AgamP4_X') %>% 
  ggplot(., aes(x=BPcum, y=Tajima, colour = Chr))+
  geom_point(size=0.3)+
  theme_classic()+
  facet_wrap(~ piece + form)+
...
#save plots
ggsave('x_tajima.tiff', agamp4_x_taj, width = 12, height = 8, units = 'in', device = grDevices::tiff)
ggsave('x_pi.tiff', agamp4_x_pi, width = 12, height = 8, units = 'in', device = grDevices::tiff)


ggsave('m_tajima.tiff', m_tajima, width = 12, height = 8, units = 'in', device = grDevices::tiff)
ggsave('s_tajima.tiff', s_tajima, width = 12, height = 8, units = 'in',device = grDevices::tiff)
ggsave('m_pi.tiff', m_pi, width = 12, height = 8, units = 'in',device = grDevices::tiff)
ggsave('s_pi.tiff', s_pi, width = 12, height = 8, units = 'in',device = grDevices::tiff)




##############################################
#between ecotype fsts
#almost same as above but data need to be cleaned a bit differently
############
# 
setwd('/Users/tristanpwdennis/Projects/MOVE/data/anopheles_03_21_DW/angsd-output/windowed_stats_1kb')


#select files, read, as usual
windowed_fst_eco_files = list.files(pattern = '*window1step0p2.txt')
...
my_threshold = quantile(winfst$fst, 0.99, na.rm = T)
